...
import { TripNarrativeBlock } from '@/types';
...
interface ViaggioStageBlockProps {
  block: TripNarrativeBlock;
  index: number;
}
...
export default function ViaggioStageBlock({
  block,
  index,
}: ViaggioStageBlockProps) {
  ...
  const gridCols = hasImage ? getGridCols(side, imageSize) : '1fr';

  const photoElement = hasImage && (
  ...
  const textElement = (
    <div className="flex items-center justify-center h-full px-8 lg:px-16 xl:px-20 py-12 lg:py-0">
      <div className="max-w-lg w-full mx-auto">
        ...
        <div className="text-base font-light leading-relaxed text-gray-600 [&>p+p]:mt-6">
          {narrative.split('\n\n').map((paragraph, i) => (
            <p key={i}>{paragraph}</p>
          ))}
        </div>
  ...
  return (
    ...
    >
      {hasImage ? (
        ...
        /* Fallback: text on left (like trails, never centered) */
