...
export default function MotoCard({ route, index }: MotoCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/moto/${route.slug}`} className="block h-full">
          ...
          <div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-all duration-300">
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <p className="text-white/70 text-sm font-mono tracking-wide drop-shadow-md group-hover:text-white/90 transition-colors">
                {route.stats.distanceKm} km · {route.stats.days} giorni
              </p>
            </div>
          </div>
        </div>
